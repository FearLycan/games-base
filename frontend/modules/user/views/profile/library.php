...
/* @var $filter frontend\modules\user\models\LibraryFilter */
...
<?php $this->beginContent('@frontend/modules/user/views/layouts/account.php'); ?>
    ...
    <div class="mb-6 fade-up" style="animation-delay:.05s">
        <h1 class="font-display text-2xl font-semibold text-fg text-balance">
            Library
            <?php if ($libraryCount): ?>
                <span class="ml-1 align-middle text-base font-medium text-fg-subtle tabular-nums">(<?= $libraryCount ?>)</span>
            <?php endif; ?>
        </h1>
        <p class="mt-1 text-sm text-fg-muted">
            ...
            <?php else: ?>
                Sync queued — your games will appear here after the next sync run.
            <?php endif; ?>
        </p>
    </div>
    ...
    <?php if ($user->isSteamLibrarySynced() && !$user->isSteamProfilePublic()): ?>
        <div class="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 fade-up" style="animation-delay:.1s">
            Your Steam profile's game details look private, so we can't read your library. In Steam → Profile → Edit Profile → Privacy, set <strong>Game details</strong> to <strong>Public</strong>, then sync again.
        </div>
    <?php endif; ?>
        ...
        <div class="rounded-2xl border border-dashed border-line-strong bg-surface/40 px-6 py-14 text-center fade-up" style="animation-delay:.12s">
            ...
            <p class="mx-auto mt-1.5 max-w-sm text-sm text-fg-muted text-pretty">
                <?= $user->isSteamLibrarySynced()
                    ? 'We synced your profile but found no owned games to show.'
                    : 'Hang tight — your library is queued for its first sync.' ?>
            </p>
        </div>
    <?php else: ?>
        <?php
        ...
        ?>
        <?= Html::beginForm(['/user/profile/library'], 'get', [
            ...
            <label class="relative min-w-[14rem] flex-1">
                ...
                <input type="search" name="q" value="<?= Html::encode($filter->q) ?>" placeholder="Search your games" aria-label="Search your games"
            ...
            <div class="relative">
                <?= Html::dropDownList('played', $filter->played, LibraryFilter::PLAYED_OPTIONS, ['class' => $selectClass, 'onchange' => 'this.form.submit()', 'aria-label' => 'Filter by play status']) ?>
                <?= $chevron ?>
            </div>

            <div class="relative">
                <?= Html::dropDownList('achievements', $filter->achievements, LibraryFilter::ACHIEVEMENTS_OPTIONS, ['class' => $selectClass, 'onchange' => 'this.form.submit()', 'aria-label' => 'Filter by achievements']) ?>
                <?= $chevron ?>
            </div>

            <div class="relative">
                <?= Html::dropDownList('sort', $filter->sort, LibraryFilter::SORT_OPTIONS, ['class' => $selectClass, 'onchange' => 'this.form.submit()', 'aria-label' => 'Sort games']) ?>
                <?= $chevron ?>
            </div>

            <noscript><button type="submit" class="inline-flex h-10 items-center rounded-lg bg-fg px-4 text-sm font-medium text-canvas">Apply</button></noscript>
        <?= Html::endForm() ?>
        ...
        <?= ListView::widget([
            ...
            'itemOptions'  => ['tag' => 'div'],
            'emptyText'    => '<div class="rounded-2xl border border-dashed border-line-strong bg-surface/40 px-6 py-14 text-center"><p class="font-display text-lg font-semibold text-fg">No matches</p><p class="mt-1.5 text-sm text-fg-muted">No games match your filters. ' . Html::a('Clear filters', ['/user/profile/library'], ['class' => 'font-medium text-accent hover:underline']) . '</p></div>',
