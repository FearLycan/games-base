...
/* Game-card hover slideshow ---------------------------------------------------
 * On hover, lazily fetch a game's screenshots and cross-fade through them over
 * the header image. Fetch only fires after a short hover-intent delay and is
 * cached per card, so brushing across a grid costs nothing. Adult screenshots
 * are gated server-side, so a card the viewer can't see returns no images.
 * -------------------------------------------------------------------------- */
(function () {
    var ENDPOINT = '/autocomplete/screenshots';
    var SLIDE_MS = 2400;   // time each screenshot is shown (matches the dot fill)
    var INTENT_MS = 130;   // hover dwell before we bother fetching

    var cards = document.querySelectorAll('[data-shots][data-shots-id]');
    if (!cards.length || !window.fetch) {
        return;
    }

    cards.forEach(function (card) {
        var id = parseInt(card.getAttribute('data-shots-id'), 10);
        if (!id) {
            return;
        }

        var shots = null;      // null = not fetched, [] = fetched/empty
        var stage = null;      // built lazily once we have images
        var dots = null;       // frame indicator, built alongside the stage
        var current = -1;      // index of the frame currently on top
        var loading = false;
        var hovering = false;
        var timer = 0;
        var intent = 0;
        ...
        function buildStage(images) {
            ...
            el.className = 'gcard-stage';
            images.forEach(function (url) {
                var img = document.createElement('img');
                img.className = 'gcard-shot';
                img.loading = 'lazy';
                img.decoding = 'async';
                img.src = url;
                el.appendChild(img);
            });
            // Insert right after the header image so a later sibling (the sale
            // badge) keeps painting on top.
            var header = card.querySelector('img');
            if (header && header.nextSibling) {
                card.insertBefore(el, header.nextSibling);
            } else {
                card.appendChild(el);
            }
            // Frame dots only make sense with more than one screenshot. The badge
            // (z-20) still paints over them (z-10).
            if (images.length >= 2) {
                dots = document.createElement('div');
                dots.className = 'gcard-dots';
                // Keep the dot-fill animation in lockstep with the slide timer.
                dots.style.setProperty('--gcard-slide', SLIDE_MS + 'ms');
                images.forEach(function () {
                    var d = document.createElement('span');
                    d.className = 'gcard-dot';
                    var fill = document.createElement('i');
                    fill.className = 'gcard-dot-fill';
                    d.appendChild(fill);
                    dots.appendChild(d);
                });
        ...
        function show(next) {
            ...
            for (var k = 0; k < imgs.length; k++) {
                if (k === next) {
                    ...
                    imgs[k].classList.add('is-on'); // fades in
                } else if (k === prev) {
            ...
            // so the cross-fade never thins out to reveal the header image beneath.
            if (prev >= 0 && prev !== next) {
                window.setTimeout(function () {
                    if (current !== prev && imgs[prev]) {
                        imgs[prev].classList.remove('is-on');
                    }
                }, 900);
            }
        }
        ...
        function start() {
            ...
            if (dots) {
                dots.classList.add('is-shown');
            }
            current = -1;
            show(0); // fades in over the (opaque) header — no gap on the first frame
            if (stage.children.length < 2) {
                return; // only one screenshot — nothing to cycle through
            }
            // Under prefers-reduced-motion the CSS drops the cross-fade, so frames
            // just swap instantly — but the slideshow still advances.
        ...
        function stop() {
            ...
            if (dots) {
                ...
                var d = dots.children;
                for (var j = 0; j < d.length; j++) {
                    d[j].classList.remove('is-on');
                }
            }
        }

        // Hold the first reveal until the leading screenshot has actually decoded,
        // so it fades in cleanly instead of popping in part-way through the fade.
        ...
        function run() {
            if (shots && shots.length) {
                if (!stage) {
                    stage = buildStage(shots);
                }
                startWhenReady();
                return;
            }
            if (shots !== null || loading) {
                return; // already known-empty, or a fetch is in flight
            }
            loading = true;
        ...
        card.addEventListener('mouseenter', function () {
            hovering = true;
            intent = window.setTimeout(run, INTENT_MS);
        });
